...
You can download the package go to it's directory and run

~~~
> cabal install
~~~

But before it you need to install one non-trivial package called FTGL.
It renders ttf-fonts wihth OpenGL. The installation is not so trivial as 
`cabal install`. But it can  be done :) 
